 ...
 * LessonVideoPlayer.tsx), by way of the OCTET module, so a learner moving
 * between them gets the same controls in the same places: dark stage, centre
 * play button, seek with a buffered track, back/forward ten, volume, elapsed,
 * speed, captions, picture-in-picture and fullscreen.
 *
 ...
 * WHERE THE FILES GO
 *
 *   apps/web/public/videos/axiom/{node-code}.mp4
 *
 * The node code names the file, so nothing has to be rewired when one
 * appears. Drop the mp4 in, rebuild the web image, and the poster is replaced
 * by the video. Until then every chapter shows the same honest poster rather
 * than hiding the slot, so the panel does not change shape on upload day.
 */
...
export default function LessonVideoPlayer({
  ...
  /** /videos/axiom/{code}.mp4, or null when the file is not uploaded yet. */
  src: string | null;
  title: string;
  poster?: string | null;
