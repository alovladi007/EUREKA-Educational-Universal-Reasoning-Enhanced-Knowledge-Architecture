"""AXIOM verified template bank, wave 2.

Generators for Calculus I-III, Probability and Statistics, and Discrete
Mathematics, plus the teaching layer: a three-rung hint ladder (orient,
method, first step) for every template in both waves, per the AXIOM Teaching
Model. The platform serves hints before any solution.

Verification stays independent of generation wherever possible:
- derivatives verified numerically against central difference quotients,
- integrals verified by differentiating the key,
- double integrals verified by integrating in the opposite order (Fubini),
- Taylor coefficients verified against the derivative formula,
- recurrence closed forms verified by iterating the recurrence,
- counting keys verified by brute-force enumeration,
- probability keys verified through sympy.stats, a separate code path.
"""
...
HINTS: dict[str, list[str]] = {
    # Wave 1: Linear Algebra
    "T-LA-2x2": [
        "You have two equations and two unknowns; the goal is the one (x, y) pair satisfying both.",
        "Eliminate one variable: scale one equation so a coefficient matches, then subtract.",
        "Multiply the first equation so its x-coefficient matches the second's, subtract, and solve the surviving one-variable equation.",
    ],
    "T-LA-matmul": [
        "You are multiplying two 2x2 matrices; the result is another 2x2 matrix.",
        "Entry (i, j) of the product is row i of A dotted with column j of B.",
        "Compute the top-left entry first: (row 1 of A) . (column 1 of B).",
    ],
    "T-LA-inverse": [
        "You need the matrix that multiplies A to give the identity.",
        "For 2x2: swap the diagonal, negate the off-diagonal, divide by the determinant.",
        "Compute det A = ad - bc first; if it is nonzero, apply the swap-negate-divide pattern.",
    ],
    "T-LA-det": [
        "You need one number summarizing the 3x3 matrix.",
        "Expand along the row or column with the most zeros, or row reduce and multiply the diagonal.",
        "Expand along row 1: a11 times its 2x2 minor, minus a12 times its minor, plus a13 times its minor.",
    ],
    "T-LA-eigen": [
        "Eigenvalues are the scalars lambda for which A - lambda I is singular.",
        "Set det(A - lambda I) = 0; for 2x2 that is lambda^2 - (trace) lambda + det = 0.",
        "Compute the trace and determinant of A, write the quadratic, and factor it.",
    ],
    "T-LA-proj": [
        "You want the shadow of b on the line through a.",
        "The projection formula: (a . b / a . a) times a.",
        "Compute the two dot products a . b and a . a first.",
    ],
    "T-LA-lsq": [
        "No line passes through all the points; you want the best-fit line.",
        "Set up A with columns (x-values, ones) and solve the normal equations A^T A beta = A^T b.",
        "Form A^T A (a 2x2 matrix) and A^T b (a 2-vector), then solve that small system.",
    ],
    # Wave 1: ODEs
    "T-OD-sep": [
        "The right side factors into (function of x)(function of y): separable.",
        "Move all y to the left, all x to the right, integrate both sides, keep the constant.",
        "Divide both sides by y (noting y = 0 separately), giving dy/y = (x-part) dx.",
    ],
    "T-OD-lin1": [
        "This is linear first order: y' + p y = q.",
        "Multiply by the integrating factor e^(int p dx); the left side becomes (mu y)'.",
        "Compute mu = e^(int p dx) for the constant p given, then rewrite the left side as a single derivative.",
    ],
    "T-OD-so-dist": [
        "Constant coefficients: exponential solutions are the ansatz.",
        "Substitute y = e^(r x) to get the characteristic quadratic in r.",
        "Write r^2 + (coefficient of y') r + (coefficient of y) = 0 and factor it.",
    ],
    "T-OD-so-ivp": [
        "First find the general solution, then fit the two constants to the two initial conditions.",
        "The characteristic roots are pure imaginary, so the general solution is C1 cos(wx) + C2 sin(wx).",
        "Apply y(0): only the cosine survives at x = 0, so C1 equals the given y(0).",
    ],
    "T-OD-laplace": [
        "You need the transform of t^n times an exponential.",
        "Use L{t^n} = n!/s^(n+1) with the s-shift: multiplying by e^(at) replaces s with s - a.",
        "Write down n!/s^(n+1) first, then substitute s -> s - a.",
    ],
    "T-OD-system": [
        "Straight-line solutions of x' = A x come from eigenpairs.",
        "Find eigenvalues from det(A - lambda I) = 0, then an eigenvector for each from (A - lambda I) v = 0.",
        "Compute trace and det of A and factor the characteristic quadratic.",
    ],
    # Wave 1: PDEs and Fourier
    "T-PF-fourier": [
        "Check the parity of f first: odd kills a0 and an; even kills bn.",
        "Apply the coefficient integrals (1/L) int f cos(n pi x / L) and the sine version over [-L, L].",
        "State which coefficients vanish by symmetry, then set up the one surviving integral.",
    ],
    "T-PF-heat": [
        "Separated heat solutions are (spatial eigenfunction)(decaying exponential in t).",
        "With zero ends the eigenfunctions are sin(n pi x / L); the decay rate is alpha (n pi / L)^2.",
        "Write sin(n pi x / L) for the given n and L, then attach exp(-alpha (n pi / L)^2 t).",
    ],
    "T-PF-wave": [
        "d'Alembert solutions are a right-mover f(x - ct) plus a left-mover g(x + ct).",
        "Any smooth shapes work; the PDE only constrains the arguments x - ct and x + ct.",
        "Write the two given shapes with arguments x - ct and x + ct and add them with the given coefficients.",
    ],
    "T-PF-harmonic": [
        "Harmonic means the Laplacian u_xx + u_yy is identically zero.",
        "Compute both second partials and add them; the check is that the sum simplifies to 0.",
        "Compute u_xx first, keeping y constant.",
    ],
    "T-PF-transport": [
        "Transport equations carry the initial profile along characteristics.",
        "Solutions of u_t + c u_x = 0 are u = f(x - ct) where f is the initial profile.",
        "Replace x in the initial condition with x - ct.",
    ],
    # Wave 2: Calculus I
    "T-C1-deriv": [
    ...
    # Wave 2: Probability and Statistics
    "T-PS-bayes": [
...
def sweep(seeds_per_template: int = 12) -> dict:
    results = {}
...
def hint_coverage() -> tuple[list[str], list[str]]:
    """Every template in both waves must carry a full 3-rung ladder."""
    from axiom_templates import REGISTRY as R1
    all_ids = set(R1) | set(REGISTRY)
    missing = sorted(tid for tid in all_ids
                     if tid not in HINTS or len(HINTS[tid]) != 3)
    orphaned = sorted(tid for tid in HINTS if tid not in all_ids)
...
if __name__ == "__main__":
    ...
    print(f"wave 2 templates: {len(res)}   variants verified: {total_ok}/{total}")
    ...
    missing, orphaned = hint_coverage()
    print(f"hint ladders: {'PASS all templates covered' if not missing else 'MISSING ' + str(missing)}"
