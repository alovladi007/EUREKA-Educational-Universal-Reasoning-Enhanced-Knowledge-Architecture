...
grade passback). Hand-written so applying it leaves zero schema drift against
the ORM models.

lti_launches.user_id carries no foreign key because OCTET owns no users table.
The value is derived from the issuer and subject at launch time.

Revision ID: 0002_lti
Revises: 0001_chemistry
...
revision: str = "0002_lti"
down_revision: str | None = "0001_chemistry"
...
def upgrade() -> None:
    ...
    op.create_index("ix_lti_launches_user_id", "lti_launches", ["user_id"])


def downgrade() -> None:
    op.drop_index("ix_lti_launches_user_id", table_name="lti_launches")
