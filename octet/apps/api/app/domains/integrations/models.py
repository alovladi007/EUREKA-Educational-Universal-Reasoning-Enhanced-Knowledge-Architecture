...
One departure from AXIOM's shape. AXIOM stores launches against a users table
it owns. OCTET owns no accounts (section 1 of the integration contract), so the
launched identity is kept as the opaque string subject OCTET's own security
layer would see on any other request, with no foreign key to follow.
"""
...
class LtiPlatform(Base):
    ...
    # An optional pinned public key (PEM) for offline and test verification.
...
class LtiLaunch(Base):
    ...
    # The OCTET user id the launch resolved to. It is derived from the issuer
    # and subject rather than owned here, so it stays stable across launches
    # without OCTET holding an account row.
