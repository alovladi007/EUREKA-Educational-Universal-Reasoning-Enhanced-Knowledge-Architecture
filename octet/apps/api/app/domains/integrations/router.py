"""Integrations routes: the LTI 1.3 handshake, platform registration, and AGS.

The handshake endpoints (jwks, login, launch) are public because they are
called by the LMS and the browser, which have no OCTET session yet. That is
exactly why the launch endpoint verifies rather than trusts: it is the one
unauthenticated path that can produce a session. Registration and AGS score
posting are gated to admin and teaching roles.

The prefix is applied where this router is mounted, following the convention in
app/api/v1.py that domain routers carry none of their own.
"""
...
@router.api_route(
    "/lti/login", methods=["GET", "POST"], summary="LTI 1.3 OIDC login init"
)
...
async def launch(request: Request, session: AsyncSession = Depends(get_session)):
    form = await request.form()
    ...
    try:
        ...
        _principal, _launch, token = await lti.provision_launch(session, platform, claims)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=f"LTI launch rejected: {exc}") from exc
    await session.commit()
    web = get_settings().web_base_url.rstrip("/")
    return RedirectResponse(url=f"{web}/learn#access_token={token}", status_code=302)


# ---------------------------------------------------------------------------
# Registration and AGS (gated)
# ---------------------------------------------------------------------------
...
async def register_platform(
    ...
    await session.flush()
    await session.commit()
    # The pinned platform key is deliberately not echoed back. Nothing on this
    # router returns key material of either party.
...
async def post_score(
    ...
    body: ScoreBody,
    session: AsyncSession = Depends(get_session),
    _teacher: Principal = Depends(teacher_only),
) -> dict:
    try:
