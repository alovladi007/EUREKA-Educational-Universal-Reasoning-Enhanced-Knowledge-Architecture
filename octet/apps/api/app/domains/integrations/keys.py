...
is fine for local testing because the kid is stable for the life of the process.

Nothing in this module returns private key material to a caller other than the
signer. public_jwks builds its response from the public numbers alone, so there
is no path by which a private component reaches an HTTP response.
"""
...
def public_jwks() -> dict:
    """The tool's active public key set, in the JWKS shape a platform expects.

    Only the key currently used for signing is published. A retired key would
    let a platform keep accepting assertions OCTET no longer issues.
    """
    numbers = _private_key().public_key().public_numbers()
    return {
        "keys": [
