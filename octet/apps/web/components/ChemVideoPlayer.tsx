...
/**
 * The computed-scene video player — the FALLBACK behind the mp4 slot.
 *
 * Uploaded files remain the primary source (the call made in 49244e14
 * stands): LessonVideoPlayer plays /videos/octet/{slug}.mp4 whenever the
 * asset exists. This player takes the stage only while no file has been
 * uploaded for a slug that has a computed scene, so the four declared slots
 * teach something today instead of showing a poster. Same chrome vocabulary
 * as the file player - dark stage, centre play button, scrub track, elapsed
 * and total time, speed, captions, fullscreen.
 *
...
export default function ChemVideoPlayer({
  ...
  const seekBy = useCallback(
    (deltaSeconds: number) => {
      setStarted(true);
      setT((prev) => Math.min(1, Math.max(0, prev + deltaSeconds / seconds)));
    },
    [seconds],
  );

  // Keyboard, matching the file player: space/k play-pause, f fullscreen,
  // r restart, arrows ±5 s, Home/End. The wrapper is focusable so the
  // shortcuts work without clicking a specific button first.
  const onKeyDown = useCallback(
    (e: React.KeyboardEvent) => {
      if (e.target instanceof HTMLInputElement) return; // the range input seeks itself
      switch (e.key) {
        case ' ':
        case 'k':
          e.preventDefault();
    ...
    [toggle, toggleFullscreen, restart, seekBy],
  );
  ...
  return (
    <div
      ...
      className={
        isFullscreen
          ? 'flex h-full w-full flex-col justify-center overflow-hidden bg-slate-950 outline-none'
          : 'overflow-hidden rounded-xl border border-border bg-slate-950 outline-none focus-visible:ring-2 focus-visible:ring-brand-500'
      }
    >
      {/* stage — in fullscreen, width is capped so the 620/300 stage also
          fits the viewport height instead of overflowing past the controls */}
      <div
        className={isFullscreen ? 'relative mx-auto w-full' : 'relative'}
        style={
          isFullscreen
            ? { maxWidth: 'min(100vw, calc((100dvh - 5rem) * 620 / 300))' }
            : undefined
        }
      >
        <svg
