/**
 * Client for EUREKA's in-app helper, called from inside OCTET.
 *
 * WHY IT TALKS TO EUREKA AND NOT TO OCTET'S OWN API
 *
 * There is one helper for the whole platform, and one support queue behind it.
 * A learner stuck in the chemistry module has the same questions as anyone
 ...
 *
 * So this is a cross-origin call to api-core. That needs OCTET's origin in
...
export function askHelp(question: string, pagePath?: string): Promise<HelpAnswer> {
  return post<HelpAnswer>('/help/ask', {
    question,
    page_path: pagePath ? `OCTET ${pagePath}` : 'OCTET',
  });
}
...
export function escalateHelp(input: {
  ...
  return post('/help/escalate', {
    ...
    page_path: input.page_path ? `OCTET ${input.page_path}` : 'OCTET',
 ...
 * OCTET those would resolve against localhost:4100 and 404, so anything that
 * is not an OCTET route is rewritten to the EUREKA origin.
 */
