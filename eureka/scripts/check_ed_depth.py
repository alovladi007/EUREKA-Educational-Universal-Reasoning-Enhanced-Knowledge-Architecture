...
The course is being taken from narrative depth to undergraduate-plus-graduate
depth one module at a time. That is a long job, and the honest way to run it is
to have a script say which modules have actually been converted rather than a
person say so. A module counts as EXPANDED when it carries all four of:

  - display equations          (>= 8 across the module)
  - generated figures          (>= 2)
  - worked examples            (>= 2)
  - a problem set with answers (>= 1 lesson containing both)

Anything short of that is reported as PARTIAL or NARRATIVE, with the missing
ingredient named, so the gap is visible in the same place the progress is.
"""
