...
Step back and notice what kind of synthesis this is. Glycogen is built with **no template**: nothing dictates where the next residue or branch goes the way a DNA strand dictates the next base. The particle's whole architecture — (α1→4) chains, branches every 8–12 residues, one buried reducing end — is written in **enzyme specificity** alone: glycogen synthase makes only (α1→4) bonds, the branching enzyme cuts and repositions blocks by its own spacing rules, glycogenin decides where a particle begins. Contrast the template-directed syntheses of the later Biochemistry II chapters, where nucleic acids and proteins take their sequence from a nucleic-acid original: polysaccharides carry no sequence information, so the same enzymes always yield statistically identical — never sequence-identical — molecules.`,
        importantNote:
