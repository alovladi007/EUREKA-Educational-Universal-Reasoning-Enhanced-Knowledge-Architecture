...
export default function ProfilePage() {
  ...
  const stats = {
    totalQuestions: user?.total_questions_answered || 247,
    accuracy: user?.overall_accuracy ? (user.overall_accuracy * 100).toFixed(1) : 78.5,
    streak: user?.current_streak_days || 7,
    studyTime: user?.total_study_time_minutes || 1240,
    rank: '#1,234',
    percentile: '85th'
  };
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl p-8 text-white">
        <div className="flex items-center justify-between">
          ...
          {!user?.is_premium && (
            <button className="bg-yellow-500 text-gray-900 px-6 py-3 rounded-lg hover:bg-yellow-400 font-semibold transition-colors">
              Upgrade to Premium
            </button>
          )}
        </div>
