...
| XR-9 | XR routes slow / dev server crash-looping (user-reported: "taking too long to load") | DONE 2026-07-14 — root cause chased through three layers with evidence: (1) `next dev` compiles routes on demand; the 3k-module three.js compiles took 3-9s cold. (2) Every rebuild/restart wiped the compile cache, re-exposing cold routes. (3) The killer: docker events showed `oom` before each `die` — the kernel was OOM-killing the dev server's compile spikes inside the 7.75GB Docker VM (39 containers), restart wiped the cache, the next click paid a cold compile, sometimes mid-compile it died again (observed 113s hang -> connection reset). A V8 heap cap and a clean-cache-on-boot + self-warming sidecar (scripts/warm-routes.mjs) reduced but could not eliminate the OOM class. FIX: the compose `web` service now serves a PRODUCTION build (Dockerfile.prod, `next build` + `next start`): every route prebuilt — measured 6-52ms per XR route, 65MiB steady memory, zero compile spikes, restarts=0. Dev mode with HMR + warmer remains available as `web-dev` (--profile dev, port 4041). Note: prod builds hard-disable dev auto-login (P1.3a by design) — sign in at /auth/login with the seeded demo account. Verified in-browser: login form -> dashboard -> Solar System portal, all sub-second. |
| XR-8 | authoring polish: unpublish + builder ergonomics | DONE 2026-07-14 — publishing was one-way; added POST /xr/scene-builder/projects/{id}/unpublish (owner-scoped, soft: linked experiences flip is_published=false so the catalog hides them while session history survives; idempotent). My Projects modal shows a Published badge + Unpublish button (publishedCount now returned per project). Builder ergonomics: Del/Backspace deletes the selection (the help overlay always promised it; input/textarea-guarded), Duplicate (deep-clones geometry+material so recolor/delete never bleed across copies), a color picker on primitives that persists through save→reopen→publish→view, and deleteObject now disposes GPU resources. Live-verified in-browser: cube→recolor #ff3300→duplicate (2 objects)→Del (1 object)→Del-while-typing is a no-op→Unpublish drops the live catalog 12→11 and the button disappears; zero console errors; next build clean. Tests: +2 (unpublish removes from catalog + idempotent; cross-user unpublish 404) → 19 XR tests pass. |
