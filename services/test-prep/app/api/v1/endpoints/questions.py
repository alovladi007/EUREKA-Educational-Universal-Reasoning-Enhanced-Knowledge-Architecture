...
async def get_questions(
    ...
    """
    Get paginated list of questions with optional filters (no auth required for development)
    """
    query = db.query(Question)
    
    # Apply filters (using actual database column names)
    if exam_type:
        query = query.filter(Question.exam == exam_type)
    if subject:
        query = query.filter(Question.section == subject)
    if topic:
    ...
    return {
        ...
        "questions": [
            {
                "id": q.id,
                "question_text": q.question_text,  # Uses compatibility property
                "options": q.options,  # Uses compatibility property
                "correct_answer": q.correct_answer,  # Uses compatibility property
